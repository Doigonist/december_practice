...
$res_products = $link -> query("SELECT * FROM `products_import`");
    ...
    <form action="addrecord.php" method="POST">
        <select name="select" id="">
            ...
            for($i = 0 ; $i < count($res_products); $i++) {
                $product = $res_products[$i]['name_product'];
                $id_prod = $res_products[$i]['id_product_import'];
